...
## 0-A. Blocker / Critical Resolution Ledger (the FAIL punch list, closed)
...
|---|---|---|---|---|---|
| B1 | Cross-tenant SSE leak — module-scoped `progressBuffer` (`packages/orchestrator/src/runtime.ts:808`, drained `:391-392`) | Move the buffer **into the per-request `run()` closure** so each request owns its own array; drain reads the closure-local buffer. GDPR Art. 5(1)(f). | `progressBuffer.concurrency.test.ts` — two tenants stream concurrently; assert run-A events NEVER surface in run-B. **Required check, gates ALL Plaid/Treasurer/iOS-Treasurer code.** | Shield + Backend lead | **Wk1 Day 1-2** (first commit) |
| B2 | iframe home (`apps/web/app/app/page.tsx:28`) + raw `layout.tsx` (renders `children` with no shell) | Delete the `<iframe>`; build native React **AppShell** and wire it into `apps/web/app/app/layout.tsx`, passing `userEmail` from the server session so **every** signed-in route sits inside the RLS-aware shell. | CI merge gate: grep-fail if `<iframe` appears under `app/app/`; a11y smoke test asserts a real nav landmark tree. | Frontend lead | **Wk2** |
| B3 | No CI at all (no `.github/workflows/`) | Add `.github/workflows/ci.yml`: `tsc --noEmit`, lint, **gitleaks**, `npm audit --audit-level=high`, **Semgrep**, `rls-proof.test.ts`, `progressBuffer` test — all **required checks**; branch protection on `main` (no direct push, 1 approval). | The workflow *is* the gate; without it no other Blocker is enforceable, so it lands Wk1. | DevOps lead | **Wk1** |
| B4 | No security headers (`apps/web/next.config.ts` has none) | Add `async headers()`: CSP (`frame-ancestors 'none'`, `frame-src cdn.plaid.com`, `connect-src` incl. `*.supabase.co` + Upstash + `cdn.plaid.com`), HSTS `preload`, `X-Frame-Options: DENY`, `X-Content-Type-Options: nosniff`, `Referrer-Policy`, `Permissions-Policy`, `Cross-Origin-Opener-Policy`. | CI job asserts **securityheaders.com grade A** against a preview URL (report-only → enforce). | Shield + Frontend lead | **Wk4** |
| B5 | Supabase auth not hardened on REMOTE (local `config.toml` doesn't sync) | Via **Management API**: `minimum_password_length=12`+complexity, `enable_confirmations=true`, TOTP enroll+verify on, Turnstile captcha, restrict `allowed_cidrs` off `0.0.0.0/0` → Railway+Vercel egress only, SSL enforce. | **Config-drift CI check** hits the Management API and fails on any regression. Gates Plaid **Production**. | Shield | **Wk3** |
| B6 | No freelancer persona | Produce `personas/autonomux-freelancer.md` from **5+ Scout interviews**: JTBD, current solution, verbatim frustrations, trigger event, device-of-first-use, financial-literacy band, **anti-persona**. | Phase 0 exit checklist requires the file to exist + Flow sign-off. **Blocks Phase 0 exit.** | Flow + Scout | **Wk2** |
| B7 | Legal sequencing inverted | A **written US-state + EU advice opinion** ("Treasurer output is informational, not regulated advice") must exist and **gate** Plaid Production submission and the App Store checklist (Guidelines **3.2.1** + **5.2.1**). | Phase 0 exit + Plaid-Production checklist both require the signed opinion on file. | Comply | **Wk1** (kick-off; opinion returns before Wk4 exit) |
| B8 | AI-interaction disclosure missing at session start; Treasurer dollar figures unlabeled | Render an **AI-interaction disclosure at every AlterEgo/Treasurer session start** on web **and** iOS; label **every** Treasurer dollar figure as AI-generated. **EU AI Act Art. 50(1) applies at Phase 1**, not Phase 2. | Added to **Phase 1 exit criteria**; eval golden set asserts every dollar figure carries the AI-generated tag. | Herald + Comply + Frontend | **Phase 1 launch** |
| B9 | C9 advice banner under-specified | Spec the **non-dismissable advice banner**: exact string at **reading grade ≤8**, placed **above the first Treasurer message** on web + iOS as a **distinct labelled UI block** (not appended text). | Design-review sign-off + Phase 1 exit check that the banner is a discrete component, not string concat. | Herald + Comply | **Before Phase 1 build** |
...
| CR2 | `agent_facts` injection (`readFactsSafely` only decrypts + truncates) | Schema-validate OR strip injection patterns, **XML-delimit** facts, cap value length before facts reach the system prompt. OWASP LLM01. | Adversarial red-team eval: **0% bypass** in CI, before Treasurer. | Shield + AI lead | Phase 0 |
...
| CR4 | userMessage cap too high (`route.ts:66`, `parseBody`) | Lower `v.userMessage.length > 200_000` → **`> 16_000`**. One-line change. | Test asserts a >16k body is rejected. | Backend lead | Wk1 |
| CR5 | Cancel button not wired (`ChatStream.tsx:586`) | Pass `onStop` to `<Composer>` (Composer already accepts `onStop` and renders the stop button at `Composer.tsx:76/197/436`). One-line fix. Satisfies cancel-on-every-streaming-surface. | UI test: stop button visible + abort fires during streaming. | Frontend lead | Wk2 |
| CR6 | `sub_agent_progress` no-op (`ChatStream.tsx:651` returns `msg`) | Append `event.message` to the matching pending sub-agent card → meets **sub-500ms visible-feedback** contract. | UI test: a progress event mutates the pending card within budget. | Frontend lead | Wk2 |

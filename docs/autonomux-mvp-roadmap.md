...
| CR3 | <span style="color:teal">✅ DONE</span> (Jury: PASS) — chat rate-limit / budget cap | **[x] Done.** `/api/chat/stream`: per-user 20/min + 200/hr (Redis, `checkChatRateLimit`) → 429 + Retry-After; per-tenant monthly LLM budget preflight (`lib/chat/budget-preflight.ts` vs `usage_meters`) → 402; `maxDuration=300`. Guards run before the stream opens. Original spec: **20/min, 200/hr** per user; **per-tenant monthly LLM budget cap** via `usage_meters` preflight returning **402**; **429 + Retry-After**; `export const maxDuration = 300`. Confirm **Vercel Pro**. | Unit + integration test: preflight over-budget → 402; over-rate → 429 with `Retry-After`. | Backend lead | Wk3 |
...
| CR7 | <span style="color:teal">✅ DONE</span> Chat breaks on mobile | **[x] Done (via prototype).** `/app` is the Claude Design prototype, which has a full mobile drawer sidebar (hamburger ≤860px) + composer-visible (`100dvh`) + safe-area + tightened spacing. The native ThreadList-collapse approach is moot; the shipped mobile chat works on iOS Safari. | Responsive check at 375/390px on the live prototype. | Frontend lead | ~~Wk2~~ **done** |
| CR8 | ⏸️ DEFERRED (iOS unbuilt) — iOS TOTP challenge missing | Spec + enforce `mfa.challengeAndVerify` for enrolled users. Without it, an iOS session is an **auth bypass on financial data**. | **Hard CI gate before TestFlight**: XCTest asserts enrolled user is challenged. | iOS lead + Shield | Before Wk8 TestFlight |
| CR9 | <span style="color:teal">✅ DONE</span> (Jury: PASS) — Plaid webhook skeleton | **[x] Done.** `/api/plaid/webhook`: reads raw body first, verifies the Plaid `Plaid-Verification` JWT (ES256, JWKS-cached) **before** any `JSON.parse`, 400 on failure, idempotency via `plaid_webhook_events` (migration 0016, service-role-only). Skeleton only (no business logic). Original spec: read **raw body FIRST**, cache JWKS 5 min, **verify signature BEFORE `JSON.parse`**, 400 on failure, idempotency on `plaid_event_id`. | Unit test: invalid header → 400 and **`JSON.parse` is never called**. Required check. | Backend lead + Shield | Phase 0 |
| CR10 | ⏸️ DEFERRED (Plaid unbuilt) — Plaid-connect trust flow | Read-only statement **before** OAuth, branded post-connect confirmation, modest first response. Test with **5 freelancers**; add a **distrust probe** to the eval set. | Eval set includes the distrust probe; UX sign-off before Phase 1 UI lock. | Canvas + Flow | Before Phase 1 UI lock |

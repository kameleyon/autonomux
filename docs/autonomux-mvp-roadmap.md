...
> **Progress update (2026-07-05 · Jury re-audit: PASS).** Code-fixable Phase-0 blockers are <span style="color:teal">**green**</span>: B1 (SSE leak), B3 (CI + Semgrep), B4 (security headers), B8-web (AI disclosure + financial guardrail), plus criticals CR2/CR4/CR5/CR6. B2 superseded by the prototype-design decision. **Remaining blockers need YOU:** B5 (apply Supabase remote auth hardening), B6 (5+ freelancer interviews for the persona), B7 (written legal advice opinion). B9 + the iOS/Treasurer half of B8 wait until those surfaces are built. Branch protection on `main` still to enable (GitHub setting).
...
**Blocker snapshot (2026-07-05, Jury re-audit PASS):** B1 <span style="color:teal">✅</span> · B2 <span style="color:teal">✅ superseded</span> · B3 <span style="color:teal">✅</span> · B4 <span style="color:teal">✅</span> · B5 ⛔ · B6 ⛔ · B7 ⛔ · B8 <span style="color:teal">✅ (web)</span> · B9 ⏸️.
...
| B3 | <span style="color:teal">✅ DONE</span> | CI / Semgrep | **[x] Done.** `ci.yml`: typecheck, lint, test, gitleaks, npm-audit, **Semgrep SAST** (`static-analysis` job, gating). ⏳ **Branch protection** still to enable (GitHub setting). | The Semgrep job gates on findings (no `continue-on-error`). | DevOps lead | ~~Wk1~~ **done (branch-protection pending)** |
...
| B5 | ⛔ BLOCKED (user) | Supabase auth not hardened on REMOTE | Via **Management API**: 12-char+complexity passwords, `enable_confirmations=true`, TOTP on, Turnstile, restrict `allowed_cidrs`. **Not provable from repo — needs live change; changes login UX, so pending user go-ahead.** | Config-drift CI check hits the Management API. | Shield | **Wk3 — awaiting user** |
| B6 | ⛔ BLOCKED (user) | No freelancer persona | Produce `personas/autonomux-freelancer.md` from **5+ Scout interviews**. **Cannot be authored from code — needs real interviews.** | Phase 0 exit requires the file + Flow sign-off. | Flow + Scout | **Wk2 — awaiting user** |
| B7 | ⛔ BLOCKED (lawyer) | Legal sequencing inverted | A **written US-state + EU advice opinion** must exist and gate Plaid Production + App Store 3.2.1/5.2.1. **Needs a lawyer.** | Phase 0 exit + Plaid checklist require the signed opinion. | Comply | **Wk1 kick-off — awaiting user** |

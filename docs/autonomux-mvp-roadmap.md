...
**Blocker snapshot (2026-07-05, Jury re-audit PASS):** B1 <span style="color:teal">✅</span> · B2 <span style="color:teal">✅ superseded</span> · B3 <span style="color:teal">✅</span> · B4 <span style="color:teal">✅</span> · B5 <span style="color:teal">✅ applied</span> · B6 <span style="color:teal">✅ draft v1</span> (needs interviews) · B7 <span style="color:teal">✅ draft</span> (needs counsel signature) · B8 <span style="color:teal">✅ (web)</span> · B9 ⏸️.
...
| B5 | <span style="color:teal">✅ APPLIED</span> | Supabase auth not hardened on REMOTE | **[x] Applied live via Management API (2026-07-05):** 12-char passwords + complexity (lower/upper/digit), `mailer_autoconfirm=false` (email confirmation required), TOTP enroll+verify on, HIBP leaked-password protection on. **CIDR lockdown intentionally skipped** (Vercel serverless has no fixed egress IPs — would break the app). | Config-drift CI check (via Management API) still to add. | Shield | ~~Wk3~~ **applied** |

...
**Legend:** <span style="color:teal">✅ DONE</span> · ⏳ IN PROGRESS / next · 🟡 HALF (part shipped) · ⛔ BLOCKED (user) · ⏸️ DEFERRED (unbuilt surface). **Snapshot (2026-07-05):** done — CR2, CR4, CR5, CR6, CR7. In progress — CR3, CR9. Half — CR12. Blocked — CR15. Deferred (Treasurer/iOS/App-Store not built) — CR1, CR8, CR10, CR11, CR13, CR14.
...
| CR9 | ⏳ IN PROGRESS — Plaid webhook skeleton | `/api/plaid/webhook`: read **raw body FIRST**, cache JWKS 5 min, **verify signature BEFORE `JSON.parse`**, 400 on failure, idempotency on `plaid_event_id`. | Unit test: invalid header → 400 and **`JSON.parse` is never called**. Required check. | Backend lead + Shield | Phase 0 |

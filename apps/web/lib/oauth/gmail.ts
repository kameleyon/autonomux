/**
 * apps/web/lib/oauth/gmail.ts
 *
 * Pure helpers around Google's OAuth 2.0 endpoints for the Gmail flow
 * (Sprint D §4 / Cluster D).
 *
 * No npm dependency: everything is `fetch` + `URLSearchParams`. Stateless —
 * the route handlers in app/auth/oauth/gmail/{start,callback}/route.ts
 * compose these with the env reader and the state-JWT helper.
 *
 * Scope policy (PRD §5 / CASA gating):
 *   - Restricted scope `gmail.modify` ONLY when
 *     GMAIL_RESTRICTED_SCOPE_APPROVED === 'true' (CASA-approved deployment).
 *   - Default falls back to `gmail.metadata` — read-only metadata, no CASA
 *     review required. The orchestrator surfaces a "limited mode" badge in
 *     this state (Acceptance #5).
 *
 * Owner: [Forge + Shield + Comply]
 */
...
export const GMAIL_SCOPE_METADATA =
  "https://www.googleapis.com/auth/gmail.metadata";

/**
 * The single Gmail scope we request, chosen at request time based on
 * GMAIL_RESTRICTED_SCOPE_APPROVED. Default = metadata (safety net).
 */
export function resolveGmailScope(): string {
  return process.env.GMAIL_RESTRICTED_SCOPE_APPROVED === "true"
    ? GMAIL_SCOPE_MODIFY
    : GMAIL_SCOPE_METADATA;
}
...
function requireEnv(name: string): string {
  ...
  if (v === undefined || v.length === 0) {
    throw new Error(`[oauth/gmail] Missing required env: ${name}`);
  }
  return v;
}
...
export function getGmailOAuthConfig(): {
  ...
  return {
    ...
    redirectUri: requireEnv("GMAIL_OAUTH_REDIRECT_URI"),
  };
}

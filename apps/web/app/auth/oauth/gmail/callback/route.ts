/**
 * apps/web/app/auth/oauth/gmail/callback/route.ts
 *
 * GET handler — Google's OAuth 2.0 redirect lands here (Sprint D §4 / Cluster D).
 *
 * Sequence (every step fails closed):
 *   1. Read `oauth_state_gmail` cookie + URL `state` param — they MUST match
 ...
 *      with `purpose:'oauth.gmail'` — the resulting envelope is the ONLY thing
 ...
 *   9. Clear the state cookie + 302 to /app/settings/integrations?connected=gmail.
 *
...
const INTEGRATION = "gmail" as const;
...
async function recordFailureEvent(args: {
  ...
  // "error" status) the gmail row for this tenant so the event chain still
